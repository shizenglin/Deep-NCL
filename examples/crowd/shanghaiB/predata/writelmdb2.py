...
def WriteLmdbTrain(imPath,dmapPath,lmdbPth,dataset,downscale,mirror,cropscale):
    """
    ...
    for idx in xrange(len(dataset)):
	    ...
	    #for sid in xrange(len(scales)):
            imArr = impro.ReadImage(imName,False)
    	    dmapArr = impro.ReadDmap(dmapName,False)
    	    data = impro.CropSubImage(imArr,dmapArr,downscale,cropscale)
    	    imArr_m = impro.ReadImage(imName,True)
    	    dmapArr_m = impro.ReadDmap(dmapName,True)
    	    data_m = impro.CropSubImage(imArr_m,dmapArr_m,downscale,cropscale)
    	    ...
    	    with imglmdb.begin(write=True) as in_txn:  
    		        for in_idx in xrange(9):
    				    ...
    				    str_id = '{:0>6d}'.format(randidx[idx*18+in_idx])#[idx*126+sid*18+in_idx]
    			  	    in_txn.put(str_id,datum.SerializeToString())
    		        for in_idx in xrange(9):
    			        datum = caffe.io.array_to_datum(data_m[in_idx])
    			        str_id = '{:0>6d}'.format(randidx[idx*18+in_idx+9])
    			        in_txn.put(str_id,datum.SerializeToString())
    	    imglmdb.close()
    	    ...
    	    with dmaplmdb.begin(write=True) as in_txn:
    		        for in_idx in xrange(9):
    				    ...
    				    str_id = '{:0>6d}'.format(randidx[idx*18+in_idx])
    				    in_txn.put(str_id,datum.SerializeToString())
    		        for in_idx in xrange(9):
...
def WriteLmdbTrainV2(imPath,dmapPath,lmdbPth,dataset,downscale,mirror):
    """
    imPath:   the folder contains images.
    dmapPath: the folder contains matfiles of density map. 
    lmdbPth:  the folder will be used to save lmdb file.
    ...
    for idx in xrange(len(dataset)):
	    ...
	    with imglmdb.begin(write=True) as in_txn:  
    		        ...
    		        str_id = '{:0>6d}'.format(randidx[idx*2])
    		        in_txn.put(str_id,datum.SerializeToString())
              
    		        datum = caffe.io.array_to_datum(imArr_m)
    		        str_id = '{:0>6d}'.format(randidx[idx*2+1])
    		        in_txn.put(str_id,datum.SerializeToString())
	    imglmdb.close()
	    dmaplmdb = lmdb.open(dmapLmdbName,map_size = int(1e12))
	    with dmaplmdb.begin(write=True) as in_txn:
